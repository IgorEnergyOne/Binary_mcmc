...
def ln_posterior(params: tuple, lightcurve: LightCurve, earth_data: ObsData, sun_data: ObsData) -> float:
    ...
    body = func.Body(ellips1, ellips2, distance=a1 + a2,
                     eq_latitude=lat,
                     eq_longitude=lon,
                     init_phase=init_phase, rotation_period=5.022 / 24)
    # initialize
...
def ln_posterior_radar(params: tuple, radar_image_params: rf.RadarImageParameters, radar_images: list, position_vecs, mult=450):
    """
    calculates chi2 for the modeled lightcurve with the given parameters
    """
    ...
    for idx_img in range(radar_image_params.N_points):
        if radar_image_params.presence[idx_img] == 1:
            phi = 2 * math.pi * (radar_image_params.jd[idx_img] - 59185.77807) / ps.P + ps.init_phase
            sinphi = math.sin(phi)
            cosphi= math.cos(phi)
